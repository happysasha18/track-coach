...
Run at Sasha's request ("прувером убедиться что ничего не ломается") after G16–G18 + the label-salad
...
- **F5 (should-clarify, invariant)** — `bass` reachable as a base-role label for an UNTRUSTED low-centroid
  ...
  the two paths to `bass`; left OPEN (ask Sasha) whether untrusted-low should read `low` instead.
