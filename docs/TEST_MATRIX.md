...
| D-INV-22 | quick-only version ⇒ cell "full analysis only" (silent, RC-INV-7); row reads the **newest** version's most-complete run (E.4, D-INV-35); never blank-implies-none | reference-column cell text: not built — lands with catalog reference render. Row = newest version: **BUILT** (D-INV-35) |
