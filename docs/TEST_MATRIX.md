...
### The aim picker & «toward X» panel (SPEC §D.6.1) — stage 1 = picker + panel; stage 2 = in-place card re-flavour
| code | rule (1-line) | owning test / status |
|---|---|---|
| D-INV-31 | **The aim panel is one named collapsible surface `#aimpanel`, default-collapsed, distinct from the in-place read.** A `<details>` placed right after `#refRead` (order: centroid line → parameters panel `#refRead` → aim panel `#aimpanel`); lists prioritized ordered steps toward the selected aim ("first Y, then Z"); never merged into the read/centroid panel. | stage 1 — to build: `test_aim_panel::AimPanelIsCollapsedDetailsAfterRefRead` (rendered HTML: `#aimpanel` is `<details>` not `open`, positioned after `#refRead`) |
| D-INV-15 (stage-2) | **Card SET identical to plain view — re-flavour never adds, removes, hides, or changes the based-on evidence.** The aimcards-block for every direction contains exactly the same cards as the baseline `#recs` (same count, same content anchors); no card is dropped or invented. | **BUILT 0.9.12** `test_aim_panel::AimCardsSetIdenticalToBaseline` (count of .rec in aimcards-block == D.recs count) |
| D-INV-17 (stage-2) | **Divergence is a secondary sort key inside the urgency tier — diverging cards rise within crit/do/concept, not across them; option-notes on diverging cards; on-style marks on matching-style cards.** In urgency mode: within each tier, cards whose axis diverges from the aim centroid (|offset| ≥ AIM_INZONE_Z) sort first; in chrono mode the lever is inert (no ties). Diverging cards get one option-note (cap=1/card, closes D-21 for single-select). On-style cards get a mark. | **BUILT 0.9.12** `test_aim_panel::AimCardsDivergingRisesInTier`; `AimCardsDivergingGetsOptionNote`; `AimCardsOnStyleMark` |
| D-INV-32 | **Build embeds, per offerable direction, the aim-panel steps (stage 1) + the §D.6 in-place re-flavoured cards (stage 2); the dropdown swaps client-side — no re-run, keyed on slug, "no aim" = baseline, not a D-INV-12 recompute input.** Every offerable direction's aim-panel block is precomputed and embedded (hidden, `data-aim="<idx>"`); the `#aimpicker` `<select>` swaps the visible block in-browser and persists to `localStorage["tc_aim_"+slug]`; "no aim" (`value=""`) shows baseline. Stage-2 rider: same embedding also carries the re-flavoured card set; `applyAim._swapCards` hides `#recs` and shows `#aimcardsDisplay` from the embedded store. | stage 1 — BUILT: `test_aim_panel::PerDirectionBlocksEmbedded`; `AimPickerPersistsBySlug`; `NoAimIsBaseline`. **Stage 2 — BUILT 0.9.12**: `AimCardsStoreEmbedded` (`#aimcardsStore` present with one `.aimcards-block` per lean); `NoAimRecs` (recs=None → no aimcardsStore) |
| D-INV-33 | **Composition across the selection states, no stranding.** (a) no aim → collapsed + empty; (b) cloud aim → full ordered steps, in-zone facets omitted; (c) reduced aim → track-vs-track steps, no «своё» talk; (d) unplaceable direction → not offered by the dropdown at all. Aim state is live only where the picker is visible; full-run only (D-INV-20); Detailed-only; nothing in Simple absent from Detailed. | stage 1 — BUILT: `test_aim_panel::AimStateComposition`; `AimPickerDetailedFullRunOnly`. **Stage 2 — BUILT 0.9.12**: `AimPickerDetailedFullRunOnly` extended — `body.simple #aimcardsDisplay` CSS gate present |
| D-INV-34 | **Every step is a real finding, ordered by divergence-toward-aim (largest gap first), never invented; "already close" when nothing moves; cites based-on.** Steps = the track's own diverging facets vs the aim centroid, sorted by |offset| desc, in-zone facets dropped; same order key as the in-place re-order (D-INV-17) so read and panel never disagree; when no facet diverges toward the aim, panel says "already close on what we can measure" — no filler; each step carries its evidence. | stage 1 — to build: `test_aim_panel::StepsAreDivergenceOrdered` (order matches |offset| desc, in-zone dropped); `StepsCiteEvidence` (each step has a based-on token); `AlreadyCloseWhenNoDivergence` (all-in-zone ⇒ the message, no invented step) |
