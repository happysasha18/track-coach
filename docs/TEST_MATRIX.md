...
| RC-INV-13a / RC-INV-13b / RC-INV-13c | run-validity WIRING — a completion re-measures the stored audio `--only-this` (no re-trigger) and carries the read forward (13a); the completeness line reads "absent in this track" (13b); `revalidate` reports incomplete runs and by default a build completes them, `--only-this` opts out (13c). | `test_pipeline_plan::RunValidityOrchestration::test_incomplete_deposits_found`, `::test_complete_run_dry_plans_reanalyse_only_this`, `::test_flags_exposed`, `test_widget_render::CompletenessLineShipped::test_line_in_markup` |
| RC-INV-13e | reference-run validity checked at direction-generation (a reference run never deposits, so the deposit gate does not reach it). | **not built** — reference runs feed centroids via `gen_reference_directions.py`, not deposit; the validity check there is a follow-on (his reference albums are already built + clean). |
