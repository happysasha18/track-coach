...
### PLANNED (not built) — source-file header symmetry & readability (Sasha 2026-06-22, NOT critical). The display path already wires both audio + .als (`build_widget.py:2276-2281`); these rows formalize the requirement + tripwire it. Both owning tests EXIST but are `@unittest.skip`ped (the suite's 2 skips) until implemented.
...
|---|---|---|
| INV-35 | **One mode at a time.** After ANY sequence of mute/solo toggles via `toggleStem`, the player is never in `(some stem muted) AND (some stem soloed)` at once — muting clears solos, soloing clears mutes (Sasha 2026-06-21). | `test_player_logic::PlayerStateMachine::test_one_mode_at_a_time` |
