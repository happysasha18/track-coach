...
| INV-46 | **Surface registry + DOM-scan convergence (s46, 2026-07-03).** `USER_SURFACES` is the single source of truth for every user-facing panel. (a) Every `<details class="tc-panel" id="…">` in a rendered full widget must be in `USER_SURFACES` — the DOM-scan test fails naming any unregistered id. (b) Every non-DEFERRED registry entry must have a real gate test method — the registry-gated check fails if a method is missing. Together: `rendered ⊆ registry ⊆ gated ⟹ rendered ⊆ gated` (100% by construction). PROOF: `test_completeness_gate::WholeArtifactCompletenessGate::test_CONV_probe_scan_detects_unregistered` injects a `__probe_unregistered` panel and verifies the scan catches it, then `test_completeness_gate::WholeArtifactCompletenessGate::test_CONV_all_rendered_panels_are_registered` verifies the real widget is clean. `refRead` / `webPanel` are §D DEFERRED and explicitly excluded from the standard 1.0 gate. Level = **browser**. | `test_completeness_gate::WholeArtifactCompletenessGate::test_CONV_probe_scan_detects_unregistered`; `test_completeness_gate::WholeArtifactCompletenessGate::test_CONV_all_rendered_panels_are_registered`; `test_completeness_gate::WholeArtifactCompletenessGate::test_CONV_every_registry_entry_has_gate_test` |
...
| DS-INV-9 | spacing split — `--gap` (within a group) vs `--rhythm` (between sections). **NOT built**: the component CSS rules still carry raw `gap:` literals and `:root` has no `--gap`/`--rhythm` tokens (verified s53). Matrix mention retires the s52 ext-namespace baseline id; coverage lands when coded. | §I.2 spacing-token pass — add the tokens to `:root`, de-literalise the rules, then a `test_design_tokens` token-defined guard |
