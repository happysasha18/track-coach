...
| RC-INV-13 / RC-INV-13a / RC-INV-13b / RC-INV-13c / RC-INV-13d / RC-INV-13e | run validity — a run reaches a surface only when complete (gate-aware, no 0.0 imputation); an invalid run never renders/deposits/feeds similarity, redone automatically once (13a), then held honest; already-deposited partial runs marked + offered for redo (13c); per-axis source→gate table (13d); completeness line reads "absent in this track" (13b); reference-run validity checked at direction-gen (13e). | **not built** — lands with the run-validity build (spec proven s65; needs gate-aware completeness + kill the `0.0` imputation in `fingerprints.py` + the render/deposit validity gate + auto-redo + the re-validate migration pass). |
