...
| INV-18 | The Evidence drawer is present and visible in EVERY view — Simple, Detailed, and quick (never CSS-hidden by `.simple`). Only its inner panels gate on data (arr/auto need `.als` — INV-16; map/rhythm/notes need stems). The Simple view hides ONLY `#stemlanes`/`#seqKey` (deep stem viz), the non-timecoded `#recs`, `#refRead` (§D.10.3 Detailed-only), and `#webPanel` (§D.10.2 Detailed-only). | `test_widget_contract::SimpleViewGating`; browser-level `test_headless_render::SimpleViewGatingBrowser::test_stem_viz_hidden_in_simple_visible_in_detailed`, `test_headless_render::SimpleViewGatingBrowser::test_ref_panels_hidden_in_simple_visible_in_detailed` |
...
<!-- REMOVED 2026-07-03 (Alexander, s49): the `#verdict` "In short" headline panel. It repeated what the recs/cards say, added visual weight to the calm-first screen, and was never on earlier versions. The verdict TEXT lives on independently in the catalog/library listing (index.json → run_dir.py:289 / library.py:288), which is untouched. Removed: the widget div + JS render + CSS + `verdict_lead` string + gate test 11. -->
...
| DS-INV-9 | spacing split — `--gap` (within a group) vs `--rhythm` (between sections). **POST-1.0 (deferred, F5/s54)** — not a 1.0 promise. Verified by deed (s54): the widget uses ~13 distinct raw `gap:` literals (2–20 px) and `:root` defines neither token, so snapping to a three-value `8/12/16` scale is a spacing NORMALISATION (a design/taste call — which of 5/6/7 px becomes 8 px is Alexander's), NOT the mechanical token-snap the motion/radii tokens were. Deferred to keep 1.0 free of a silent restyle; SPEC §I.2 now marks it POST-1.0. Coverage lands with that design pass. | POST-1.0 §I.2 spacing-token design pass — normalise the scale (Alexander's call), add `--gap`/`--rhythm` to `:root`, de-literalise, then a `test_design_tokens` token-defined guard |
